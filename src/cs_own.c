...
void cs_deown(char *src, int ac, char **av) {
	...
	ChanInfo *c;
	if(ac<3) {
		notice(cs_name,src,CS_ERR_ACCCMD_USAGE,"DEOWN");
	...
	if(level_src<OWNER_LIST) {
		notice(cs_name,src,CS_ERR_XOP_HIGHERACCESS,"Owner");
		return;
	}
	if(level_target<OWNER_LIST) {
...
void cs_own(char *src, int ac, char **av) {
	...
	ChanInfo *c;
	if(ac<3) {
		notice(cs_name,src,CS_ERR_ACCCMD_USAGE,"OWN");
