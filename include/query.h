...
const char *load_ns_access = "SELECT NICKS.NICKNAME,NS_ACCESS.MASK FROM NS_ACCESS"
		"INNER JOIN NICKS ON NS_ACCESS.NICK_ID = NICKS.NICK_ID;";

const char *load_ns_notify = "SELECT NICKS.NICKNAME,NS_NOTIFY.MASK FROM NS_NOTIFY"
		"INNER JOIN NICKS ON NS_NOTIFY.NICK_ID = NICKS.NICK_ID;";

const char *load_ns_auth = "SELECT NS_AUTH.TARGET_TYPE, "
		"CASE WHEN NS_AUTH.TARGET_TYPE = 1 THEN NOTIFY.NICKNAME ELSE CHANS.NAME END AS TARGET_NAME,"
		"SENDER.NICKNAME, RECIPIENT.NICKNAME,TIME,STATUS"
		"FROM NS_AUTH"
